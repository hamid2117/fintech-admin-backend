...
router.post(
  '/agency',
  asyncHandler(async (req, res) => {
    const { codee, label, status, currency, state } = req.body
    ...
    const createdCash = await Agency.save()
    if (createdCash) {
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    const cash = await Agency.findById(req.params.id)
    if (cash) {
      Agency.label = req.body.label || Agency.label
      Agency.status = req.body.status || Agency.status
      const updatedcash = await Agency.save()
